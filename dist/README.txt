...
In order to run this program you must first have the appropriate JOGL jars and
libraries for you operating system. The necessary files can be downloaded from 

http://jogamp.org/deployment/webstart/archive/

In order to run the program, the lib folder and at least jogl.all.jar and gluegen.jar
must be on the Java classpath. The lib folder is found at
/jogamp-<your_platform>-<your_processor_type>/lib, and the jar files are at
/jogamp-<your_platform>-<your_processor_type>/jar

For more information
on setting up the runtime environment, see the setup tutorials at

https://jogamp.org/wiki/index.php/Jogl_Tutorial

The executable must also be run from a folder containing the accompanying resources
folder.
